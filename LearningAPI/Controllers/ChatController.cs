...
namespace BlissAPI.Controllers
{
    ...
    public class ChatController : ControllerBase
    {
        ...
        [HttpGet("supportTicket/{supportTicketId}")]
        [ProducesResponseType(typeof(IEnumerable<Chat>), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public IActionResult GetChatsBySupportTicket(int supportTicketId)
        {
            try
            {
                var chats = _context.Chats.Where(c => c.SupportTicketId == supportTicketId).ToList();
            ...
            {
                _logger.LogError(ex, "Error when getting chats by support ticket id");
                return StatusCode(500, "Internal server error");
            }
        }
